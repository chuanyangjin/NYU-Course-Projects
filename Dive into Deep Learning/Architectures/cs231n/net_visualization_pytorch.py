...
def make_fooling_image(X, target_y, model):
    ...
    for step in range(num_iterations):
        ...
        if torch.argmax(score) == target_y:
            print(f"Successfully fool the model after {step} iterations.")
            break
        ...
        if step % 1 == 0:
            with torch.no_grad():
                softmax = torch.softmax(score, 1).squeeze()
                print("step: {:>2}, predicted {:>3}, target {}, target score: {:.2f}%"\
                      .format(step, torch.argmax(score), target_y, softmax[target_y]*100))
